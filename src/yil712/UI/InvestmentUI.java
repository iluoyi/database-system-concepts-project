...
public class InvestmentUI extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					InvestmentUI frame = InvestmentUI.getInstance();
	...
	private static InvestmentUI singleton = null;	// singleton
	public static InvestmentUI getInstance() {
		if (singleton == null) {
			singleton = new InvestmentUI();
		}
			...
			public void actionPerformed(ActionEvent e){
				if (invesetRow.isEmpty()) {
					...
					if (table.getSelectedRow() < 0) {
						...
						BuyInvestmentUI ui = BuyInvestmentUI.getInstance(symbol, accountID);
						ui.setVisible(true);
					}
				}
			}
			...
			public void actionPerformed(ActionEvent e){
				if (invesetRow.isEmpty()) {
					...
					if (table.getSelectedRow() < 0) {
						...
						String symbol = invesetRow.get(index).get(0).trim();
						String accountID = ManagementUI.getInstance().getAccountNum();
						SellInvestmentUI ui = SellInvestmentUI.getInstance(symbol, accountID);
						ui.setVisible(true);
					}
				}
			}
