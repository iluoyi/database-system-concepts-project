...
import yil712.control.InvestAcctControl;
import yil712.control.SymbolControl;

public class SellInvestmentUI extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					SellInvestmentUI frame = SellInvestmentUI.getInstance("AAN", "A000006");
	...
	private static SellInvestmentUI singleton = null;	// singleton
	public static SellInvestmentUI getInstance(String symbol, String accountNum) {
		if (singleton == null) {
			singleton = new SellInvestmentUI(symbol, accountNum);
		} else {
			symbolStr = symbol;
	...
	public void upateInfo() {
		...
		InvestAcctControl control2 = new InvestAcctControl();
		...
		InvestAcctControl control3 = new InvestAcctControl(); 
		double bal = control3.getInvestAccountBalance(accountID);
	 ...
	 * Create the frame.
	 */
	private SellInvestmentUI(String symbol, String accountNum) {
		symbolStr = symbol;
		accountID = accountNum;
		setResizable(false);
